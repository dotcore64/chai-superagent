...
const normalizeAddress = ({ address, family, port }) =>
  `http://${
    SUPERAGENT_VERSION === "9"
...
describe("superagent", () => {
  ...
  describe("Browser and Node.js", () => {
    it("can request a web page", () =>
      request.get(`${BASEURL}/html`).then((res) => {
    ...
    it("can request JSON data", () =>
      request.get(`${BASEURL}/get`).then((res) => {
        expect(res).to.have.status(200);
    ...
    it("succeeds when response has an error status", () =>
      request
    ...
    it("should reject non valid instances in strict mode", () => {
      expect(() => expect({}).to.have.status(200)).to.throw(
        Error,
        "expected {} to be an instance of Response",
      );

      expect(() => expect({}).to.be.json).to.throw(
        Error,
        "expected {} to be an instance of Request or Response",
      );

      expect(() => expect({}).to.be.html).to.throw(
        Error,
        "expected {} to be an instance of Request or Response",
      );

      expect(() => expect({}).to.be.text).to.throw(
        Error,
        "expected {} to be an instance of Request or Response",
      );

      expect(() => expect({}).to.have.header("Pragma", "test1")).to.throw(
        Error,
        "expected {} to be an instance of Request or Response",
      );

      expect(() => expect({}).to.redirect).to.throw(
        Error,
        "expected {} to be an instance of Response",
      );

      expect(() => expect({}).to.redirectTo("foo")).to.throw(
        Error,
        "expected {} to be an instance of Response",
      );

      expect(() => expect({}).to.have.param("foo", "bar")).to.throw(
        Error,
        "expected {} to be an instance of Request",
      );

      expect(() => expect({}).to.have.cookie("foo", "bar")).to.throw(
        Error,
        "expected {} to be an instance of Request",
      );
      ...
      expect(() => expect({}).to.have.charset("utf8")).to.throw(
        Error,
        "expected {} to be an instance of Request",
      );
    });
