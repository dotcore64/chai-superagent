...
describe("assertions", () => {
  ...
  it("#redirectTo", () => {
    ...
    expect({ status: 301, headers: { location: "bar" } }).to.not.redirectTo(
      "foo",
    );
    ...
    expect({ status: 200, redirects: ["foo/bar?baz=qux"] }).to.redirectTo(
      /^foo\/bar/,
    );

    expect(() => {
      expect({ status: 301, headers: { location: "foo" } }).to.not.redirectTo(
        "foo",
      );
    }).to.throw("expected header 'location' to not have value foo");

    expect(() => {
      expect({ status: 301, headers: { location: "bar" } }).to.redirectTo(
        "foo",
      );
